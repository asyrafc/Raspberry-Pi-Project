...
int main(void)
{
    ...
    unsigned char tx_buffer2[] = "AT+CMGS=\"+60178727038\"\r\n";
	...
	{	
		...
		strftime (out2, 5, "%H", now_tm);
		strftime (outM, 5, "%M", now_tm);
		strftime (outS, 5, "%S", now_tm);
		strftime (outD, 5, "%d", now_tm);
		strftime (outMon,5, "%m", now_tm);
		strftime (outY, 5, "%y", now_tm);
		int outInt = atoi(out2);
		int outIntM = atoi(outM);
		int outIntS = atoi(outS);
		int outIntD = atoi(outD);
		int outIntMon = atoi(outMon);
		int outIntY = atoi(outY);
		//printf("%i", outInt);
		...
		char dataWithTime[20];
		//char smsString[1024];
		snprintf(dataToString,10,"%.2f",data2);
		snprintf(dataWithTime,20,"%.2f @ %02d:%02d:%02d",data2,outInt,outIntM,outIntS);
		...
		memcpy(&tx_buffer3[42],&dataWithTime, 16);
		...
		if(data2 > 36.0)
        {
			...
			printf("PPM : %f", data2);
			printf(" @ %02d-%02d-%02d|%02d:%02d:%02d\n",outIntY,outIntMon,outIntD,outInt,outIntM,outIntS);
			
			sprintf(timeString," %s",out);
			sprintf(q,"Insert into status (ppm,time,status) values ('%s','%s','Gas Leakage Detected')",dataToString,timeString);
        ...
        {
            printf("Safe level of gas PPM : %f", data2);
			printf(" @ %02d-%02d-%02d|%02d:%02d:%02d\n",outIntY,outIntMon,outIntD,outInt,outIntM,outIntS);
			printText(fd,lcdBuffer2);
			softToneWrite(BUZZER_PIN, 0);
			
			sprintf(timeString," %s",out);
			sprintf(q,"Insert into status (ppm,time,status) values ('%s','%s','Safe level of gas')",dataToString,timeString);
